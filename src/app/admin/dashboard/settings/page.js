...
import { getSiteConfig, updateSiteConfig } from '@/lib/firebaseUtils';
        ...
        <div>
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
                <div>
                    ...
                    <p className="text-xs text-[#7A7268] mt-1 font-sans">Saved to Firestore — changes apply site-wide</p>
                </div>
                <button onClick={handleSave} disabled={isSaving}
                    className={`flex items-center px-6 py-2.5 rounded text-sm uppercase tracking-wider cursor-pointer transition-all disabled:opacity-60 ${saved ? 'bg-green-600 text-white' : 'bg-[#C9A96E] text-[#0D0B09] hover:bg-[#F5F0E8]'}`}>
            ...
            <div className="flex flex-col lg:flex-row gap-6 max-w-5xl">
                ...
                <div className="flex-1 bg-white rounded shadow border border-[#D9D0C0] p-6 sm:p-8">
                    <div className="flex justify-between items-center mb-6 pb-4 border-b border-[#D9D0C0]">
                        ...
                        <button onClick={() => handleReset(activeTab)} className="flex items-center text-xs text-[#7A7268] hover:text-[#8B4A2F] cursor-pointer transition-colors">
                    ...
                    <div className="space-y-6">
                        {currentTab?.fields.map(field => {
                            ...
                            if (field.type === 'toggle') {
                                return (
                                    <label key={field.key} className="flex items-center gap-3 cursor-pointer">
                                        <div className={`w-11 h-6 rounded-full relative transition-colors ${settings[field.key] ? 'bg-[#C9A96E]' : 'bg-[#D9D0C0]'}`}
                            ...
                            if (field.type === 'textarea') {
                                return (
                                    <div key={field.key}>
                                        ...
                                        <textarea className="w-full border border-[#D9D0C0] p-2.5 rounded focus:border-[#C9A96E] outline-none resize-none text-sm"
                                            rows={3} value={settings[field.key]} onChange={e => update(field.key, e.target.value)} placeholder={field.placeholder} />
                            ...
                            return (
                                <div key={field.key}>
                                    <label className="block text-xs font-medium text-[#7A7268] uppercase mb-1.5">{field.label}</label>
                                    <input type="text" className="w-full border border-[#D9D0C0] p-2.5 rounded focus:border-[#C9A96E] outline-none text-sm"
                                        value={settings[field.key]} onChange={e => update(field.key, e.target.value)} placeholder={field.placeholder} />
                                    {field.hint && <p className="text-[10px] text-[#7A7268] mt-1">{field.hint}</p>}
                                </div>
                            );
                        })}
